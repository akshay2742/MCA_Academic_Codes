...
void push()
{
    if(!isfull())
    {
        ...
        printf("Enter the Marks: ");        //push a student record
    ...
    else
    {
        ...
        stack = (struct student*)calloc(size*2,sizeof(struct student));     //updating the size of the array by 1
        for(int i=0;i<size;i++)
        {
        ...
        free(temp);
        temp = NULL;         //updating the stack size by doubling it using realloc
...
int main()
{
    ...
    do
    {
        printf("\n!-----------------Program to implement stack using dynamic array and increasing the size by double-----------------!\n");
